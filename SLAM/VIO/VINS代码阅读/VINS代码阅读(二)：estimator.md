...
​	我们可以看到，单线程中，每当图像信息到时，才进入函数`processMeasurements`；多线程中，循环提取featureBuf中的front，没有则跳过，有则进行处理。**总结：图像特征到后才进行一次测量处理。**
